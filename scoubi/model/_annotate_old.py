...
def train(adata, axon_markers, dendrite_markers, pairs = None, epochs = 500, lambda_gex = 1e-1, lambda_comm = 1, ftemp = 1e-1, patience = 50, lr = 1e-3, device='cpu', weight_decay=1e-4, seed = 42, tol = 1e-2, do_comm = True):
    ...
    for epoch in pbar:
        ...
        comm_term = (ct_loss_for_n1 / len(pairs)).item() if do_comm else ct_loss_for_n1
        pbar.set_description(
            f"Epoch {epoch+1}/{epochs} | Total: {total_loss.item():.6f} | GEX: {gex_loss_term.item():.6f} | Comm: {comm_term:.6f} | Patience: {patience_counter}/{patience}"
        )
    ...
    with torch.no_grad():
        ...
        Z_score = model.score(x_all).cpu().numpy()
